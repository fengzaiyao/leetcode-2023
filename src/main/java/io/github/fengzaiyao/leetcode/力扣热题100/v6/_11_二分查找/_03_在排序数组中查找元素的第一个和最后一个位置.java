package io.github.fengzaiyao.leetcode.力扣热题100.v6._11_二分查找;
...
public class _03_在排序数组中查找元素的第一个和最后一个位置 {
    ...
    public static int[] searchRange(int[] nums, int target) {
        int l = doSearchRange(nums, target, true);
        int r = doSearchRange(nums, target, false);
        return new int[]{l, r};
    }

    public static int doSearchRange(int[] nums, int target, boolean isLeft) {
        int l = 0;
        int r = nums.length - 1;
        while (l <= r) {
            int mid = (l + r) / 2;
