package io.github.fengzaiyao.leetcode.力扣热题100.v2._04_子串;

import java.util.HashMap;
import java.util.Map;

// https://leetcode.cn/problems/minimum-window-substring/?envType=study-plan-v2&id=top-100-liked

public class _03_1111_最小覆盖子串 {

    public static void main(String[] args) {
        System.out.println(minWindow("ADOBECODEBANC", "ABC"));
        System.out.println(minWindow("a", "a"));
    }

    public static String minWindow(String s, String t) {
        Map<Character, Integer> needMap = new HashMap<>();
        Map<Character, Integer> currMap = new HashMap<>();
        char[] chars = t.toCharArray();
        for (char ch : chars) {
            needMap.put(ch, needMap.getOrDefault(ch, 0) + 1);
        }
        int l = 0;
        int r = 0;
        String result = null;
        while (r < s.length()) {
            if (!isCover(needMap, currMap)) {
                currMap.put(s.charAt(r), currMap.getOrDefault(s.charAt(r), 0) + 1);
                r++;
            }
            while (isCover(needMap, currMap)) {
                String substring = s.substring(l, r);
                if (result == null) {
                    result = substring;
                } else if (result.length() > substring.length()) {
                    result = substring;
                }
                currMap.put(s.charAt(l), currMap.getOrDefault(s.charAt(l), 0) - 1);
                l++;
            }
        }
        return result == null ? "" : result;
    }

    private static boolean isCover(Map<Character, Integer> needMap, Map<Character, Integer> currMap) {
        for (Map.Entry<Character, Integer> entry : needMap.entrySet()) {
            if (!currMap.containsKey(entry.getKey()) || currMap.get(entry.getKey()) < entry.getValue()) {
                return false;
            }
        }
        return true;
    }
}
