package io.github.fengzaiyao.leetcode.力扣热题100.v7._11_二分查找;
...
public class _03_在排序数组中查找元素的第一个和最后一个位置 {
    ...
    public static int[] searchRange(int[] nums, int target) {
        int l = searchIdx(nums, target, true);
        int r = searchIdx(nums, target, false);
        return new int[]{l, r};
    }

    public static int searchIdx(int[] nums, int target, boolean isLeft) {
        int l = 0;
        int r = nums.length - 1;
        while (l <= r) {
            int mid = (l + r) / 2;
